...
def parse_args():
    ...
    parser.add_argument(
        '--pr_model_path', type=Path, help='Path to the Phoneme Recognizer model.',
        default=Path('../models/w2v2_phon_rec/wav2vec2-xlsr-53'),
    )
    parser.add_argument(
    ...
    if args.exp_dir is None:
        # Train from scratch
        args.exp_dir = Path(f'../experiments/FORCE_APTAI/{args.date_time}_{args.exp_name}')
...
def _collate_fn(batch):
    ...
    return {
        ...
        'phoneme_labels' : torch.nn.utils.rnn.pad_sequence(
            [torch.IntTensor(x['phoneme_label']) for x in batch],
            batch_first=True,
            padding_value=-100,
        ),
...
def _prepare_model_optim_scheduler(args_cfg):
    model = force_aptai.Force_APTAI(
        args_cfg.pr_model_path,
        args_cfg.device,
        args_cfg.vocab,
    ).to(args_cfg.device)
...
def train(
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        for batch_idx, batch_x in enumerate(train_dataloader):
            ...
            train_tv_loss = outputs['tv_loss']
            train_align_loss = outputs['align_loss']
            train_loss.backward()
            ...
            log_str = (
                ...
                f'train_tv_loss: {train_tv_loss:.4f}, '
                f'train_align_loss: {train_align_loss:.4f}, '
                f'lr: {curr_lr:.6f}'
            )
            ...
            if cfg.logging and is_wandb_available():
                ...
                wandb.log({'train_tv_loss': train_tv_loss})
                wandb.log({'train_align_loss': train_align_loss})
...
def validate(
    ...
    val_losses, val_tv_losses, val_align_losses = [], [], []
    val_rmses, val_pccs = [], []
    ...
    for batch_idx, batch_x in enumerate(val_dl):
        ...
        with torch.no_grad():
            ...
            val_tv_loss = outputs['tv_loss'].item()
            val_align_loss = outputs['align_loss'].item()
            val_losses.append(val_loss)
            val_tv_losses.append(val_tv_loss)
            val_align_losses.append(val_align_loss)
            ...
            # PER (ctc based)
            gt_phn_seq_idx = batch_x['phoneme_labels'].cpu().numpy()[0]
            pred_phn_seq_idx = outputs['pred_ctc_phn_seq'][0].tolist()
            val_edit_distance = editdistance.eval(gt_phn_seq_idx, pred_phn_seq_idx)
            val_edit_distances.append(val_edit_distance)
            val_num_phoneme = len(gt_phn_seq_idx)
            val_num_phonemes.append(val_num_phoneme)
            val_per = val_edit_distance / val_num_phoneme
            
            # FER: 1 - (correct_pred_frames / total_frames)
            frames = batch_x['phn_frames_49hz'].size(1)
            val_total_frames += frames
            gt_frames = batch_x['phn_frames_49hz']
            pred_frames = torch.tensor(outputs['pred_frame_phns'], device=device)
            elem_equal = torch.eq(gt_frames, pred_frames)
            corr_frames = torch.sum(elem_equal).item()
            val_corr_frames += corr_frames
            cur_val_fer = 1 - (corr_frames / frames)
            ...
            # TODO same for aptai model
            # TODO this is not correct yet
            # TODO not frames but boundaries!
                # have method that converts frames to boundries
                    # call this for gt_frames and pred_frames
                # then put this in the get_stats method
            y = gt_frames.detach().cpu().numpy().squeeze()
            # print('gt_frames', y)
            yhat = pred_frames.detach().cpu().numpy().squeeze()
            # print('pred_frames', yhat)
            p, r, f1, rval = get_stats(y, yhat, tolerance=0.02)
            val_ps.append(p)
            val_rs.append(r)
            val_f1s.append(f1)
    ...
    return {
        ...
        'val_mean_PER': np.sum(val_edit_distances) / np.sum(val_num_phonemes),
...
def test(
    ...
    test_rmses, test_pccs, test_pers = [], [], []
    ...
    for batch_idx, batch_x in enumerate(test_dl):
        ...
        with torch.no_grad():
            ...
            gt_phn_seq_idx = batch_x['phoneme_labels'].cpu().numpy()[0]
            pred_phn_seq_idx = outputs['pred_ctc_phn_seq'][0].tolist()
            test_edit_distance = editdistance.eval(gt_phn_seq_idx, pred_phn_seq_idx)
            test_edit_distances.append(test_edit_distance)
            test_num_phoneme = len(gt_phn_seq_idx)
            ...
            # FER: 1 - (correct_pred_frames / total_frames)
            frames = batch_x['phn_frames_49hz'].size(1)
            test_total_frames += frames
            gt_frames = batch_x['phn_frames_49hz']
            pred_frames = torch.tensor(outputs['pred_frame_phns'], device=device)
            elem_equal = torch.eq(gt_frames, pred_frames)
            corr_frames = torch.sum(elem_equal).item()
            test_corr_frames += corr_frames
            cur_test_fer = 1 - (corr_frames / frames)
    ...
    return {
        ...
        f'test_{rate}_std_rmse': np.array(list(mean_test_rmse_TVs.values())).std(),
        f'test_{rate}_mean_pcc': np.array(list(mean_test_pcc_TVs.values())).mean(),
        f'test_{rate}_std_pcc': np.array(list(mean_test_pcc_TVs.values())).std(),
        ...
        f'test_{rate}_mean_PER': np.sum(test_edit_distances) / np.sum(test_num_phonemes),
        f'test_{rate}_std_PER': np.std(test_pers),
        f'test_{rate}_mean_overlap': np.mean(test_overlaps),
        f'test_{rate}_std_overlap': np.std(test_overlaps),
...
if __name__ == '__main__':
    ...
    for loso_idx in range(0, 8):
        ...
        best_model = force_aptai.Force_APTAI(
            cfg.pr_model_path,
            cfg.device,
            cfg.vocab,
        ).to(cfg.device)
